...
using Unity.Mathematics;                  // Handles
...
namespace FischlWorks_FogWar
{
    ...
    public class csFogWar : MonoBehaviour
    {
        ...
        [SerializeField]
        private float fogLerpMaxLimit = 0.01f;
        [Header("Debug")]
        [SerializeField]
        ...
        //[SerializeField]
        //private float rayMaxDistance = 10;//2d就用不上了
        [SerializeField]
        private LayerMask ScanLayers = new LayerMask();
        [SerializeField]
        ...
        SortingGroup fogPlaneSortingGroup;
        ...
        private void InitializeVariables()
        {
            ...
            {
                ScanLayers = LayerMask.GetMask("Default");
            }
        ...
        private void InitializeFog()
        {
            ...
            fogPlane.name = "[RUNTIME] Darkness_Plane";

            fogPlaneSortingGroup = fogPlane.AddComponent<SortingGroup>();
            if (SortingLayer.NameToID("Front") != -1)
            {
                // 获取新的Sorting Layer的ID
                int newSortingLayerID = SortingLayer.NameToID("Front");

                // 设置游戏对象的Sorting Layer
                fogPlaneSortingGroup.sortingLayerID = newSortingLayerID;
            }
        ...
        private void UpdateFog()
        {
            fogPlane.transform.position = new Vector3(
                levelMidPoint.position.x,
                levelMidPoint.position.y,
                levelMidPoint.position.z + fogPlaneHeight);//这个以后再看
        ...
        private void UpdateFogPlaneTextureBuffer()
        {
            for (int xIterator = 0; xIterator < levelDimensionX; xIterator++)
            {
                for (int yIterator = 0; yIterator < levelDimensionY; yIterator++)
                {
                    ...
                    // Calculate the color difference between the current color and target color
                    float colorDifference = Vector4.Distance(bufferPixel, targetPixel);
                    
                    if(colorDifference <= fogLerpMaxLimit)
                    {
                        fogPlaneTextureLerpBuffer.SetPixel(xIterator, yIterator, bufferPixel + lastspeed[xIterator][yIterator]);
                    }
                    else
                    {
                        lastspeed[xIterator][yIterator] = Color.Lerp(bufferPixel, targetPixel, fogLerpSpeed * Time.deltaTime);
                        fogPlaneTextureLerpBuffer.SetPixel(xIterator, yIterator, lastspeed[xIterator][yIterator]);
                    }
                }
            }
        ...
        private void ScanLevel()
        {
            ...
            Vector2 boxCastSize = new Vector2((unitScale - scanSpacingPerUnit) / 2.0f,
            ...
            for (int xIterator = 0; xIterator < levelDimensionX; xIterator++)
            {
                ...
                for (int yIterator = 0; yIterator < levelDimensionY; yIterator++)
                {
                    Vector2 circleCenter = new Vector2(GetWorldX(xIterator), GetWorldY(yIterator));

                    Collider2D collider = Physics2D.OverlapBox(circleCenter, boxCastSize, ScanLayers);
        ...
        public bool RemoveFogRevealer(int revealerIndex)
        {
        ...
        public bool CheckVisibility(Vector3 worldCoordinates, Vector2Int additionalAera,bool isQualityHigh,LayerMask CheckLayers)
        {
            Vector2Int levelCoordinates = WorldToLevel(worldCoordinates);

            Vector2 CastBox = new Vector2((unitScale - scanSpacingPerUnit) / 2.0f,
                                         (unitScale - scanSpacingPerUnit) / 2.0f);

            if (additionalAera.x == 0 && additionalAera.y == 0)
            {
            ...
            for (int xIterator = -1; xIterator < additionalAera.x + 1; xIterator++)
            {
                for (int yIterator = -1; yIterator < additionalAera.y + 1; yIterator++)
                {
                    ...
                    if(shadowcaster.fogField[levelCoordinates.x + xIterator][levelCoordinates.y + yIterator] ==
                        Shadowcaster.LevelColumn.ETileVisibility.Revealed)
                    {
                        if (!isQualityHigh)
                        {
                        ...
                        {
                            Collider2D collider = Physics2D.OverlapBox(worldCoordinates, CastBox, CheckLayers);
                            if(collider!=null)
                            {
                                scanResult = true;
                            }
